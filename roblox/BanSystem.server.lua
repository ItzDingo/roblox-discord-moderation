...
local function execute(data)
 ...
 if data.action=="ban" then
  local ok,err=pcall(function()Players:BanAsync({UserIds={userId},ApplyToUniverse=true,Duration=-1,DisplayReason=tostring(data.reason),PrivateReason="Discord moderation ban #"..tostring(data.banId),ExcludeAltAccounts=false,ApplyDeviceBlock=true})end)
  if ok then api("/api/complete","POST",{banId=data.banId,status="active"});announce({displayName=data.displayName,reason=data.reason}) else warn("BanAsync failed",err) end
 elseif data.action=="unban" then
  local ok,err=pcall(function()Players:UnbanAsync({UserIds={userId},ApplyToUniverse=true})end)
  if ok then api("/api/complete","POST",{banId=data.banId,status="unbanned"}) else warn("UnbanAsync failed",err) end
 end
end
